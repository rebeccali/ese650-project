...
from environments import simplequad_mpc_params
import pdb
...
class SimpleQuadEnv(gym.Env):
    ...
    def __init__(self):
        self.dt = simplequad_mpc_params.dt
        self.timesteps = simplequad_mpc_params.timesteps
        self.total_time = simplequad_mpc_params.total_time

        self.g = simplequad_mpc_params.gr  # gravity
        self.J = simplequad_mpc_params.J  # moment of inertia
        self.L = simplequad_mpc_params.L  # length (m) from COM to thrust point of action
        self.m = simplequad_mpc_params.m

        self.states = simplequad_mpc_params.states
        self.num_controllers = simplequad_mpc_params.num_controllers

        # ddp parameters
        self.Q_r_ddp = simplequad_mpc_params.Q_r_ddp
        self.Q_f_ddp = simplequad_mpc_params.Q_f_ddp
        self.R_ddp = simplequad_mpc_params.R_ddp
        self.gamma = simplequad_mpc_params.gamma
        self.num_iter = simplequad_mpc_params.num_iter

        # state and control spaces
        self.state_limits = np.ones((simplequad_mpc_params.states,), dtype=np.float32) * 10000
        ...
        self.state = np.zeros((simplequad_mpc_params.states,))
        # state: x,y,z, dx,dy,dz, r,p,y, dr,dp,dy

        self.goal = simplequad_mpc_params.xf
    ...
    def step(self, u):
        ...
        state_dot = np.zeros((simplequad_mpc_params.states,))
    ...
    def state_control_transition(self, x, u):
        ...
        m = simplequad_mpc_params.m
        L = simplequad_mpc_params.L
        J = simplequad_mpc_params.J
        Jx = J[0, 0]
        Jy = J[1, 1]
        Jz = J[2, 2]

        states = simplequad_mpc_params.states
        controllers = simplequad_mpc_params.num_controllers
